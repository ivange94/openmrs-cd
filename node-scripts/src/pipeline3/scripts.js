  ...
  * @param {String} source - The source path to be linked.
  ...
  dockerApacheMacro: {
    createProxy(
      ...
      var apacheConfFolder = module.exports.trailSlash(proxy.confFolder, true) + "use_macro";
      var sudo = "sudo su"
      ...
      var semanage = "";
      if (selinux == "true") {
        semanage = "semanage port -a -t http_port_t -p tcp " + proxy.port + "\n";
      }
    ...
    reload(proxy) {
      return module.exports.docker.exec(proxy.containerName, "apachectl graceful");
    },
    maintenance(mode, proxy) {
      ...
      return (
        ...
        array[1] +
        "/g' " +
        proxy.confFolder + "/use_macro/" + apacheConfFile +
        "\n" +
        module.exports.dockerApacheMacro.reload(proxy)
      );
    }
